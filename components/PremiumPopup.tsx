...
export const PremiumPopup: React.FC<PremiumPopupProps> = ({ user }) => {
  ...
  if (!popup || !user || !isVisible) return null;
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative w-full max-w-lg overflow-hidden bg-[#050505] border border-purple-500/30 rounded-3xl shadow-[0_0_50px_rgba(168,85,247,0.15)] animate-in zoom-in-95 duration-500">
        {/* Background Glows */}
        ...
        <div className="relative p-8 md:p-12 text-center">
          ...
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-1 mb-6 bg-purple-900/30 border border-purple-500/30 rounded-full">
          ...
          {/* Title */}
          <h2 className="text-3xl md:text-5xl font-black mb-6 bg-gradient-to-b from-white via-purple-200 to-purple-500 bg-clip-text text-transparent leading-tight tracking-tight uppercase italic">
          ...
          {(config?.video_url || config?.youtube_id) && (
            <div className="mt-8 relative aspect-video rounded-2xl overflow-hidden border border-purple-500/20 bg-black shadow-2xl">
              {config.video_url ? (
                <video 
